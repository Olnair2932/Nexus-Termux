...
// ============================================================
// EXECUTOR GENÉRICO DE SKILLS PYTHON
// Executa diretamente qualquer skill Python registrada
// em skills.json, passando a solicitação como argumento.
// ============================================================
if (
    ...
    try {
        const caminhoSkillsPython = path.join(
            CONFIG.ROOT,
            "skills.json"
        );

        const dadosSkillsPython = JSON.parse(
            fs.readFileSync(caminhoSkillsPython, "utf8")
        );

        const skillDireta =
            dadosSkillsPython.skills?.[intent.acao];

        if (
            skillDireta &&
            skillDireta.executor === "python" &&
            skillDireta.script
        ) {
            const scriptDireto = path.resolve(
                CONFIG.ROOT,
                skillDireta.script
            );

            const pastaFerramentasDireta = path.resolve(
                CONFIG.ROOT,
                "nexus_tools"
            );

            if (
                !scriptDireto.startsWith(
                    pastaFerramentasDireta + path.sep
                )
            ) {
                respostaFinal = "Execução bloqueada.";
            } else if (!fs.existsSync(scriptDireto)) {
                respostaFinal =
                    "Script inexistente: " +
                    skillDireta.script;
            } else {
                const argumentoDireto =
                    intent.params ||
                    promptUsuario ||
                    "";

                const argumentosDiretos =
                    argumentoDireto.trim()
                        ? ` ${JSON.stringify(argumentoDireto.trim())}`
                        : "";
                ...
                respostaFinal = execSync(
                    `python3 "${scriptDireto}"${argumentosDiretos}`,
                    {
                        cwd: CONFIG.ROOT,
                        encoding: "utf8",
                ...
                console.log(
                    "✅ Skill Python executada:",
                    intent.acao
                );
            }
        }
    } catch (erroSkillDireta) {
        respostaFinal =
            "Erro ao executar skill Python: " +
            erroSkillDireta.message;
    }
}
